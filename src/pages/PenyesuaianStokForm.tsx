  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold tracking-tight">{tt('Penyesuaian Stok Baru')}</h1>
      </div>
              ...
              items.map((it) => (
                <KartuBaris key={it.id}>
                  ...
                  <BarisInfo
                    label="Qty"
                    value={
                      <span className={it.qty < 0 ? 'text-destructive' : 'text-emerald-600'}>
                        {it.qty > 0 ? '+' : ''}
                        {angka(it.qty)}
                      </span>
                    }
                  />
                  <BarisInfo label="HPP" value={it.hpp_satuan ? angka(it.hpp_satuan) : '-'} />
                </KartuBaris>
              ))
            )}
          </DaftarMobile>
