...
function FormBaru() {
  ...
  const [fakturList, setFakturList] = useState<FakturRingkas[]>([])
  ...
  async function simpan() {
    setError(null)
    if (!pelangganId || !header.gudang_id) {
      setError(new Error('Pilih pelanggan dan gudang dulu.'))
      return
    }
    setMenyimpan(true)
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          faktur_id: header.faktur_id || null,
          gudang_id: header.gudang_id,
          masuk_stok: header.masuk_stok,
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" asChild>
          <Link to="/retur-penjualan">
            <ArrowLeft className="h-4 w-4" />
          </Link>
        </Button>
        <h1 className="text-2xl font-bold tracking-tight">{tt('Retur Penjualan Baru')}</h1>
      </div>
      ...
      <Card>
        <CardContent className="space-y-4 p-4">
          ...
          {pelangganId && fakturList.length > 0 ? (
            <div className="space-y-1.5">
              <Label>Faktur asal (opsional)</Label>
              <Select value={header.faktur_id} onChange={(e) => setHeader((h) => ({ ...h, faktur_id: e.target.value }))}>
                <option value="">Tanpa faktur</option>
                {fakturList.map((f) => (
                  <option key={f.id} value={f.id}>
                    {f.nomor} ({fmtTanggal(f.tanggal)})
                  </option>
                ))}
              </Select>
            </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" asChild>
              <Link to="/retur-penjualan">Batal</Link>
            </Button>
...
function FormEdit({ returId }: { returId: string }) {
  ...
  const { data: retur, isLoading, error: errorMuat } = useQuery({
    queryKey: ['retur-jual-detail', returId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('retur_penjualan')
        .select(
  ...
  const { data: items } = useQuery({
    queryKey: ['retur-jual-item', returId],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  function invalidateSemua() {
    queryClient.invalidateQueries({ queryKey: ['retur-jual-item', returId] })
    queryClient.invalidateQueries({ queryKey: ['retur-jual-detail', returId] })
    queryClient.invalidateQueries({ queryKey: ['retur-penjualan'] })
  }
  ...
  async function tambahItem() {
    ...
    setMenambah(true)
    try {
      const { error } = await supabase.from('retur_penjualan_item').insert({
        retur_id: returId,
  ...
  async function ubahStatus(statusBaru: 'selesai' | 'dibatalkan') {
    if (statusBaru === 'dibatalkan' && !(await konfirmasi(tt('Batalkan retur ini? Efek stoknya akan dibalik.'), { berbahaya: true }))) return
    setError(null)
