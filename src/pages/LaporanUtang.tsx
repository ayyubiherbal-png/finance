...
import type { VHutang, VHutangAging } from '@/types/db'

const UKURAN_HALAMAN = 50

type FilterUmur = '' | 'belum_jatuh_tempo' | '1-30' | '31-60' | '61-90' | '90+'

const KOLOM_UMUR: Record<Exclude<FilterUmur, ''>, keyof VHutangAging> = {
...
function useHutangAging(cari: string, umur: FilterUmur, halaman: number) {
  return useQuery({
    queryKey: ['laporan-hutang-aging', cari, umur, halaman],
    queryFn: async () => {
      let q = supabase.from('v_hutang_aging').select('*', { count: 'exact' })
      if (cari.trim()) q = q.ilike('nama_supplier', `%${cari.trim()}%`)
...
function useRingkasanHutang() {
  return useQuery({
    queryKey: ['laporan-hutang-ringkasan'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('v_hutang_aging')
        .select('total_hutang,belum_jatuh_tempo,umur_1_30,umur_31_60,umur_61_90,umur_90_plus')
        .returns<VHutangAging[]>()
      if (error) throw error
      return data ?? []
    },
...
function useHutangJatuhTempo(cari: string, umur: FilterUmur, halaman: number) {
  return useQuery({
    queryKey: ['laporan-hutang-jatuh-tempo', cari, umur, halaman],
    queryFn: async () => {
      if (umur === 'belum_jatuh_tempo') return { baris: [] as VHutang[], total: 0 }
      let q = supabase.from('v_hutang').select('*', { count: 'exact' }).neq('bucket_umur', 'belum_jatuh_tempo')
      if (cari.trim()) {
        const pola = kutipFilterPostgrest(`%${cari.trim()}%`)
        q = q.or(`nomor.ilike.${pola},nama_supplier.ilike.${pola}`)
      }
...
const KOLOM_EKSPOR_AGING: KolomEkspor<VHutangAging>[] = [
  { header: 'Supplier', nilai: (r) => r.nama_supplier },
  { header: 'Total', nilai: (r) => r.total_hutang, format: (v) => rupiah(v as number), rata: 'kanan' },
...
const KOLOM_EKSPOR_LEWAT_TEMPO: KolomEkspor<VHutang>[] = [
...
export function LaporanUtang() {
  ...
  const { data: aging, isLoading, error, isFetching } = useHutangAging(cari, umur, halamanAging)
  const { data: ringkasan } = useRingkasanHutang()
  const { data: lewatTempo, error: errorLewatTempo } = useHutangJatuhTempo(cari, umur, halamanLewatTempo)
  ...
  const total = {
    hutang: (ringkasan ?? []).reduce((t, r) => t + Number(r.total_hutang), 0),
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{tt('Laporan Utang')}</h1>
          <p className="text-sm text-muted-foreground">{tt('Sisa tagihan supplier berdasarkan umur jatuh tempo')}</p>
        </div>
        {ringkasan && ringkasan.length > 0 ? (
          <TombolEkspor
            ambilData={async () => {
              let q = supabase.from('v_hutang_aging').select('*')
              if (cari.trim()) q = q.ilike('nama_supplier', `%${cari.trim()}%`)
              if (umur) q = q.gt(KOLOM_UMUR[umur], 0)
              const { data, error } = await q.order('total_hutang', { ascending: false }).limit(10000).returns<VHutangAging[]>()
              if (error) throw error
              return data ?? []
            }}
            kolom={KOLOM_EKSPOR_AGING}
            opsi={{ namaFile: `hutang-aging-${tanggalISO()}`, judul: tt('Laporan Utang') }}
          />
        ) : null}
      </div>
      ...
      {isLoading ? (
        ...
        <KondisiKosong pesan="Tidak ada utang berjalan." />
      ) : (
        <>
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <KartuAging judul="Total" nilai={total.hutang} />
          ...
          <Card>
            <CardContent className="p-0 pb-2">
                  ...
                  <Tbody>
                    {aging?.baris.map((r) => (
                      <Tr key={r.supplier_id}>
                        <Td className="font-medium">{r.nama_supplier}</Td>
                        <Td className="tabular text-right font-medium">{rupiah(r.total_hutang)}</Td>
              ...
              <DaftarMobile>
                {aging?.baris.map((r) => (
                  <KartuBaris key={r.supplier_id}>
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate font-medium">{r.nama_supplier}</p>
                      <p className="tabular shrink-0 font-semibold">{rupiah(r.total_hutang)}</p>
                    </div>
              ...
              {aging?.baris.length === 0 ? <KondisiKosong pesan="Tidak ada utang yang cocok dengan filter." /> : null}
          ...
          {lewatTempo && lewatTempo.total > 0 ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                ...
                <TombolEkspor
                  ambilData={async () => {
                    let q = supabase.from('v_hutang').select('*').neq('bucket_umur', 'belum_jatuh_tempo')
                    if (cari.trim()) {
                      const pola = kutipFilterPostgrest(`%${cari.trim()}%`)
                      q = q.or(`nomor.ilike.${pola},nama_supplier.ilike.${pola}`)
                    }
                    if (umur) q = q.eq('bucket_umur', umur)
                    const { data, error } = await q.order('hari_lewat', { ascending: false }).limit(10000).returns<VHutang[]>()
                    if (error) throw error
                    return data ?? []
                  }}
                  kolom={KOLOM_EKSPOR_LEWAT_TEMPO}
                  opsi={{ namaFile: `hutang-lewat-tempo-${tanggalISO()}`, judul: tt('Faktur lewat jatuh tempo') }}
                />
              </div>
              <Card>
                <CardContent className="p-0 pb-2">
                      ...
                      <Tbody>
                        {lewatTempo.baris.map((f) => (
                          <Tr key={f.faktur_id}>
                            ...
                            <Td className="font-medium">{f.nama_supplier}</Td>
                  ...
                  <DaftarMobile>
                    {lewatTempo.baris.map((f) => (
                      <KartuBaris key={f.faktur_id}>
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate font-medium">{f.nama_supplier}</p>
