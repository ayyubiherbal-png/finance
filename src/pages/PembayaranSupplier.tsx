...
function useDaftarBayar(cari: string, status: string, halaman: number) {
  return useQuery({
    queryKey: ['pembayaran-supplier', cari, status, halaman],
    queryFn: async () => {
      const mulai = (halaman - 1) * UKURAN_HALAMAN
      let q = supabase.from('pembayaran_supplier').select('id, nomor, tanggal, metode, jumlah, status, supplier:supplier_id(nama)', { count: 'exact' })
      ...
      return daftarBerhalaman((data ?? []) as unknown as BarisBayar[], count)
    },
...
export function PembayaranSupplier() {
  ...
  const { data, isLoading, error, isFetching } = useDaftarBayar(cari, status, halaman)
