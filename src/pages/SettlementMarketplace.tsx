...
export function SettlementMarketplace() {
  ...
  const daftar = useQuery({
    queryKey: ['settlement-marketplace', halaman],
    queryFn: async () => {
      const mulai = (halaman - 1) * UKURAN_HALAMAN
      const { data, count, error } = await supabase.from('settlement_marketplace')
        ...
        .order('tanggal', { ascending: false }).range(mulai, mulai + UKURAN_HALAMAN - 1)
  ...
  return <div className="space-y-4">
    ...
    {bukaForm ? <Card><CardContent className="space-y-4 p-4">
      ...
      <div><Label>Pesanan dalam pencairan</Label>{pesanan.isLoading ? <Spinner /> : !pesanan.data?.length ? <p className="mt-2 text-sm text-muted-foreground">{tt('Tidak ada pesanan')} {kanal} {tt('yang masih menunggu settlement.')}</p> : <div className="mt-2 max-h-64 overflow-auto rounded-xl border"><Table><Thead><Tr><Th className="w-10"></Th><Th>Pesanan</Th><Th>Faktur</Th><Th className="text-right">Sisa bruto</Th></Tr></Thead><Tbody>{pesanan.data.map((x) => <Tr key={x.id}><Td><input type="checkbox" className="h-4 w-4 accent-primary" checked={pilihan.has(x.id)} onChange={(e) => setPilihan((lama) => { const baru = new Set(lama); e.target.checked ? baru.add(x.id) : baru.delete(x.id); return baru })} /></Td><Td>{x.nomor_pesanan_platform}</Td><Td>{x.faktur?.nomor}</Td><Td className="text-right tabular">{rupiah((x.faktur?.total ?? 0) - (x.faktur?.terbayar ?? 0))}</Td></Tr>)}</Tbody></Table></div>}</div>
